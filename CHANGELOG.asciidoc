...
* Converted Unix line endings to Windows line endings in text files, so Windows
  users doesn't need to ditch their beloved notepad (source files unchanged).
* Fix CoM dissapearing due to engines of outdated mods.
